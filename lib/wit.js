...
function Wit(opts) {
  ...
  const {
    accessToken, apiVersion, actions, headers, logger, witURL, lowConfidenceThreshold
  ...
  const continueRunActions = (sessionId, currentRequest, message, prevContext, i) => {
    return (json) => {
      ...
      if ( confidence && confidence <= lowConfidenceThreshold) {
        logger.debug(`entity confidence is too low (${confidence}<=${lowConfidenceThreshold})`)
        return runAction(actions, 'lowConfidence', request).then(ctx => {
...
const validate = (opts) => {
  ...
  opts.lowConfidenceThreshold = opts.lowConfidenceThreshold || DEFAULT_LOW_CONFIDENCE_THRESHOLD;
